...
class AGP:
    def __init__(self, func: callable, a: float, b: float, r: float = 2.5, eps: float = 1e-3):
        self.func = func
        self.a = a
        self.b = b
        self.r = r
    ...
    def calculateR(m, z_opt, interval) -> float:
        return (interval[1] - interval[0]) + ((interval[3] - interval[2]) ** 2) / (
                (m**2) * (interval[1] - interval[0])) - 2 * (interval[3] + interval[2] - 2 * z_opt) / m
    ...
    def run(self):
        self.clear()
        interval_t = (self.a, self.b,
                      self.func(self.a), self.func(self.b))
        # interval[0] = x0, interval[1] = x1, interval[2] = z0, interval[3] = z1
        M = self.r * self.calculateM(interval_t)
        if M < 1e-8:
            M = 1
        z_opt_prev = 0
        while (interval_t[1] - interval_t[0]) > self.eps * (self.b - self.a):
