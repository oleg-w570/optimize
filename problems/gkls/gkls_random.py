...
class GKLSRandomGenerator:
    ...
    def __init__(self):
        self.rnd_num = np.zeros(
            GKLSRandomGenerator.KK, dtype=np.double
        )  # array of random numbers */
        self.ran_u = np.zeros(
            GKLSRandomGenerator.LL, dtype=np.double
        )  # the generator state */
    ...
    def Initialize(
        ...
        u = np.zeros(
            [GKLSRandomGenerator.KK + GKLSRandomGenerator.KK - 1], dtype=np.double
        )
        ul = np.zeros(
            [GKLSRandomGenerator.KK + GKLSRandomGenerator.KK - 1], dtype=np.double
        )
        ...
        for j in range(
            GKLSRandomGenerator.KK, GKLSRandomGenerator.KK + GKLSRandomGenerator.KK - 1
        ):
            u[j] = ul[j] = 0.0
        ...
        while t > 0:
            ...
            for j in range(
                GKLSRandomGenerator.KK + GKLSRandomGenerator.KK - 2,
                GKLSRandomGenerator.KK - GKLSRandomGenerator.LL,
                -2,
            ):
                ul[GKLSRandomGenerator.KK + GKLSRandomGenerator.KK - 1 - j] = 0.0
                u[GKLSRandomGenerator.KK + GKLSRandomGenerator.KK - 1 - j] = (
                    u[j] - ul[j]
                )

            for j in range(
                GKLSRandomGenerator.KK + GKLSRandomGenerator.KK - 2,
                GKLSRandomGenerator.KK - 1,
                -1,
            ):
                if ul[j] != 0:
                    ul[j - (GKLSRandomGenerator.KK - GKLSRandomGenerator.LL)] = (
                        ulp - ul[j - (GKLSRandomGenerator.KK - GKLSRandomGenerator.LL)]
                    )
                    u[
                        j - (GKLSRandomGenerator.KK - GKLSRandomGenerator.LL)
                    ] = GKLSRandomGenerator.mod_sum(
                        u[j - (GKLSRandomGenerator.KK - GKLSRandomGenerator.LL)], u[j]
                    )
                    ul[j - GKLSRandomGenerator.KK] = (
                        ulp - ul[j - GKLSRandomGenerator.KK]
                    )
                    u[j - GKLSRandomGenerator.KK] = GKLSRandomGenerator.mod_sum(
                        u[j - GKLSRandomGenerator.KK], u[j]
                    )
            ...
            if GKLSRandomGenerator.is_odd(s) != 0:  # "multiply by z" */
                ...
                if ul[GKLSRandomGenerator.KK]:
                    ...
                    u[GKLSRandomGenerator.LL] = GKLSRandomGenerator.mod_sum(
                        u[GKLSRandomGenerator.LL], u[GKLSRandomGenerator.KK]
                    )
    ...
    def GenerateNextNumbers(self):
        ...
        for j in range(GKLSRandomGenerator.KK, n):
            self.rnd_num[j] = GKLSRandomGenerator.mod_sum(
                self.rnd_num[j - GKLSRandomGenerator.KK],
                self.rnd_num[j - GKLSRandomGenerator.LL],
            )
        j = j + 1

        for i in range(GKLSRandomGenerator.LL):
            self.ran_u[i] = GKLSRandomGenerator.mod_sum(
                self.rnd_num[j - GKLSRandomGenerator.KK],
                self.rnd_num[j - GKLSRandomGenerator.LL],
            )
            j = j + 1
